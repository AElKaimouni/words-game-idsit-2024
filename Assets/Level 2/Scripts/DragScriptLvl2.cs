...
public class DragScriptLvl2 : MonoBehaviour, IBeginDragHandler, IDragHandler, IEndDragHandler, IDropHandler
{
    public static DragScriptLvl2 hurufSedangDrag;
    ...
    public void OnDrop(PointerEventData eventData)
    {
        if (petunjuk && !terisi)
        {
            Debug.Log(hurufSedangDrag.Huruf.ToString());
            Debug.Log(Huruf.ToString());

            if (Huruf == "?" && hurufSedangDrag.Huruf == GameManagerLvl2.targetLetter.ToString())
            {
                GameManagerLvl2.Instance.NextLevel();
